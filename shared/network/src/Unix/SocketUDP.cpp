...
std::vector<unsigned char> &SocketUDP::read(struct sockaddr **so)
{
...
void SocketUDP::write(const std::vector<unsigned char> &buff, struct sockaddr *so)
{
  if (buff.size() != 0 && so)
  {
...
bool SocketUDP::_read()
{
  ...
  if ((len = recvfrom(_socketPoll.getFd(), &buffer.front(), buffer.size(), 0, (struct sockaddr*)(&addr), &addrlen)) == 0)
    return false;
...
bool SocketUDP::_write()
{
  if (!_listOut.empty())
  {
    ...
    if (sendto(_socketPoll.getFd(), &it->first.front(), it->first.size(), 0, it->second, sizeof(struct sockaddr_storage)) == -1)
      return false;
    _listOut.pop_front();
  }
  if (_listOut.empty())
