...
Player::Player()
{
  this->_type = AElement::PLAYER;
}

Player::Player(uint32_t idFrom, uint32_t id, uint16_t x, uint16_t y, uint16_t hp, uint16_t sizex, uint16_t sizey, uint16_t damage, float angle, char speed) : GameElement::GameElement(idFrom, id, AElement::PLAYER, x, y, hp, sizex, sizey, damage, angle, speed)
{
	_alive = true;
}
